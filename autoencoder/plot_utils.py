...
def plot_worst(sess, data_train, X, output, hyperparams):
    ...
    for x, y in np.ndindex((2, 3)):
        axes[x, y].plot(data_train[idx_sq[- (x + y + 1)]], label="Input")
        axes[x, y].plot(output_train[idx_sq[- (x + y + 1)]], label="Reconstruction")
        axes[x, y].set_title("Index {}".format(idx_sq[- (x + y + 1)]))
